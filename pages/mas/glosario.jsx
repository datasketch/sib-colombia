...
import glosary from '../../static/data/glosario.json'
import { AppContext } from '../_app'

export default function glosario (data) {
  const [word, setWord] = useState(glosary)
  const { setFooterBgColor } = useContext(AppContext)

  const handleChange = ({ target }) => {
    const filter = word.filter(({ termino }) => termino.toLowerCase().includes(target.value.toLowerCase()))
    ...
    <>
      ...
      <div className='max-w-screen-2xl mx-auto'>
        ...
        <div className='mx-auto w-9/12 grid grid-cols-2 gap-8 py-8'>
          {word.map(({ termino: title, definicion }, i) =>
            <CardGlosario title={title} definition={definicion} key={title} />
          )}
        </div>
      </div>
    </>
