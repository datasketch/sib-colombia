...
export async function getDepartmentsPath () {
  ...
  return files.filter(filename => {
    return !filename.includes('colombia') && !filename.includes('reserva-forestal-la-planada') && !filename.includes('resguardo-indigena-pialapi-pueblo-viejo')
  }).map(filename => ({
    params: {
      regiones: filename
    }
...
export async function getMunicipalitesPath () {
  ...
  const files = contents
    .filter(filename => {
      return filename.includes('/') && !filename.endsWith('_map') && !filename.endsWith('.svg') && !filename.endsWith('.geojson')
    })
    .map(filename => {
