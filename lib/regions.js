export const fs = require('fs').promises
const path = require('path')

const dataDir = path.resolve(process.cwd(), 'public', 'data')

// TODO: check nesting level for early exit
async function readMunicipalities (root, prefix = '') {
  const filesList = await fs.readdir(root, { withFileTypes: true })
  const res = []
  for (const dirent of filesList) {
    const isDir = dirent.isDirectory()

    res.push(prefix + dirent.name)

    if (isDir) {
      const inner = await readMunicipalities(path.resolve(root, dirent.name), `${dirent.name}/`)
      res.push(...inner)
    }
  }
  return res.map(item => item.replace('.json', ''))
}

export async function getDepartmentsPath () {
  const files = await fs.readdir(dataDir)
  return files.filter(filename => {
    return !filename.includes('colombia') &&
           !filename.includes('reserva-forestal-la-planada') &&
           !filename.includes('resguardo-indigena-pialapi-pueblo-viejo') &&
           !filename.includes('region-amazonia') &&
           !filename.includes('.DS_Store') &&
           !filename.includes('__MACOSX') &&
           !filename.startsWith('.') &&
           !filename.endsWith('.json') // Exclude standalone JSON files
  }).map(filename => ({
    params: {
      regiones: filename
    }
  }))
}

export async function getMunicipalitesPath () {
  const contents = await readMunicipalities(dataDir)
  const files = contents
    .filter(filename => {
      return filename.includes('/') &&
             !filename.endsWith('_map') &&
             !filename.endsWith('.svg') &&
             !filename.endsWith('.geojson') &&
             !filename.includes('.DS_Store') &&
             !filename.includes('__MACOSX') &&
             !filename.includes('region-amazonia') // Exclude region-amazonia directory
    })
    .map(filename => {
      const [regiones, municipio] = filename.split('/')
      return {
        regiones, municipio
      }
    })
    .filter(file => file.regiones !== file.municipio)
    // Exclude paths that have static page files to avoid conflicts
    .filter(file => !(file.regiones === 'narino' &&
                     (file.municipio === 'reserva-forestal-la-planada' ||
                      file.municipio === 'resguardo-indigena-pialapi-pueblo-viejo')))

  const paths = files.map(filename => ({
    params: filename
  }))
  return paths
}

export async function getDepartmentData (slug) {
  const content = await fs.readFile(path.join(dataDir, `${slug}/${slug}.json`), 'utf8')
  return content
}

export async function getMunicipalityData (region, slug) {
  // Pull from nested folder: public/data/{region}/{slug}.json
  const content = await fs.readFile(path.join(dataDir, `${region}/${slug}.json`), 'utf8')
  return content
}

export async function getStandaloneRegionData (slug) {
  // Pull from standalone folder: public/data/{slug}/{slug}.json
  const content = await fs.readFile(path.join(dataDir, `${slug}/${slug}.json`), 'utf8')
  return content
}

export async function getMapData (slug) {
  try {
    // Try to read .geojson file first
    const content = await fs.readFile(path.join(dataDir, `${slug}/${slug}.geojson`), 'utf8')
    return content
  } catch (error) {
    // If .geojson doesn't exist, fall back to _map.json
    if (error.code === 'ENOENT') {
      const content = await fs.readFile(path.join(dataDir, `${slug}/${slug}_map.json`), 'utf8')
      return content
    }
    throw error
  }
}
